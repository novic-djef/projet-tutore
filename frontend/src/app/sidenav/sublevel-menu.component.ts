...
@Component({
  ...
  template: `
    <ul *ngIf="collapsed && data.items && data.items.length > 0" [@submenu]="expanded
    ? {value: 'visible',
     ...
     class="sublevel-nav">
       <li *ngFor="let item of data.items" class="sublevel-nav-item">
        <a class="sublevel-nav-link"
           (click)="handlClick(item)" *ngIf=" item.items && item.items.length > 0" [ngClass]="getActiveClass(item)">
